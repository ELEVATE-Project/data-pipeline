...
class DiscussionStreamFunctionTestSpec extends BaseTestSpec {
  ...
  implicit val eventTypeInfo: TypeInformation[DiscussionEvent] = TypeExtractor.getForClass(classOf[DiscussionEvent])
  ...
  def initialize(): Unit = {
    when(mockKafkaUtil.kafkaJobRequestSource[DiscussionEvent](jobConfig.discussionInputTopic))
      .thenReturn(new DiscussionEventSource)
    when(mockKafkaUtil.kafkaStringSink(jobConfig.discussionOutputTopic))
      .thenReturn(new GenerateDiscussionSink)
  }
