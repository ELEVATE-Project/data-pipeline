...
class MentoringStreamFunctionTestSpec extends BaseTestSpec {
  ...
  val config: Config = ConfigFactory.parseFile(new File("unified-common.conf")).resolve().withFallback(ConfigFactory.systemEnvironment())
  ...
  "Mentoring Stream Job " should "execute successfully " in {
    initialize()
    UnifiedStreamTask.runJob(jobConfig, mockKafkaUtil, "mentoring")
  }
