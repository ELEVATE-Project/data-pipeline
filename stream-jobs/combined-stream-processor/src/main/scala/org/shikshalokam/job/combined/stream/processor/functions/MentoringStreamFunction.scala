...
class MentoringStreamFunction(config: UnifiedStreamConfig)(implicit val mapTypeInfo: TypeInformation[MentoringEvent], @transient var postgresUtil: PostgresUtil = null)
  ...
  private def checkExistenceOfDataAndPushMessageToKafka(resultList: List[String], context: ProcessFunction[MentoringEvent, MentoringEvent]#Context, tableName: String, tenantCode: String): Unit = {
  ...
  private def pushMentoringDashboardEvents(dashboardData: util.HashMap[String, String], context: ProcessFunction[MentoringEvent, MentoringEvent]#Context): util.HashMap[String, AnyRef] = {
