...
class ObservationStreamFunction(config: UnifiedStreamConfig)(implicit val mapTypeInfo: TypeInformation[ObservationEvent], @transient var postgresUtil: PostgresUtil = null)
  ...
  override def processElement(event: ObservationEvent, context: ProcessFunction[ObservationEvent, ObservationEvent]#Context, metrics: Metrics): Unit = {
    ...
    if (event.status.toLowerCase() == "started" || event.status.toLowerCase() == "inprogress" || event.status.toLowerCase() == "submitted" || event.status.toLowerCase() == "completed") {
      ...
      logger.info(s"statusOfSubmission = $statusOfSubmission")
      logger.info(s"submissionId = $submissionId")
      logger.info(s"userId = $userId")
      logger.info(s"submissionNumber = $submissionNumber")
      logger.info(s"userOneProfileName = $userOneProfileName")
      logger.info(s"userOneProfileId = $userOneProfileId")
      logger.info(s"userTwoProfileName = $userTwoProfileName")
      logger.info(s"userTwoProfileId = $userTwoProfileId")
      logger.info(s"userThreeProfileName = $userThreeProfileName")
      logger.info(s"userThreeProfileId = $userThreeProfileId")
      logger.info(s"userFourProfileName = $userFourProfileName")
      logger.info(s"userFourProfileId = $userFourProfileId")
      logger.info(s"userFiveProfileName = $userFiveProfileName")
      logger.info(s"userFiveProfileId = $userFiveProfileId")
      logger.info(s"themes = $themes")
      logger.info(s"criteriaEvent = $criteriaEvent")
      logger.info(s"solutionId = $solutionId")
      logger.info(s"solutionName = $solutionName")
      logger.info(s"programName = $programName")
      logger.info(s"programId = $programId")
      logger.info(s"observationName = $observationName")
      logger.info(s"observationId = $observationId")
      logger.info(s"tenantId = $tenantId")
      logger.info(s"solutionExternalId = $solutionExternalId")
      logger.info(s"solutionDescription = $solutionDescription")
      logger.info(s"programExternalId = $programExternalId")
      logger.info(s"programDescription = $programDescription")
      logger.info(s"privateProgram = $privateProgram")
      logger.info(s"projectCategories = $projectCategories")
      logger.info(s"projectDuration = $projectDuration")
      logger.info(s"completedDate = $completedDate")
      logger.info(s"domainTable = $domainTable")
      logger.info(s"questionTable = $questionTable")
      logger.info(s"statusTable = $statusTable")
      logger.info(s"isRubric = $isRubric")
      logger.info(s"entityType = $entityType")
      logger.info(s"entityId = $entityId")
      logger.info(s"entityName = $entityName")
      logger.info(s"entityExternalId = $entityExternalId")
      logger.info(s"parentOneObservedName = $parentOneObservedName")
      logger.info(s"parentOneObservedId = $parentOneObservedId")
      logger.info(s"parentTwoObservedName = $parentTwoObservedName")
      logger.info(s"parentTwoObservedId = $parentTwoObservedId")
      logger.info(s"parentThreeObservedName = $parentThreeObservedName")
      logger.info(s"parentThreeObservedId = $parentThreeObservedId")
      logger.info(s"parentFourObservedName = $parentFourObservedName")
      logger.info(s"parentFourObservedId = $parentFourObservedId")
      logger.info(s"parentFiveObservedName = $parentFiveObservedName")
      logger.info(s"parentFiveObservedId = $parentFiveObservedId")
      logger.info("\n\n")
       ...
       */
      if (solutionId != null) {
        logger.info("===> Processing observation solution data")
      ...
      if (statusOfSubmission != null) {
        logger.info("===> Processing observation status data")
      ...
      if (statusOfSubmission == "completed") {
        logger.info("===> Processing observation domain data")
        if (isRubric) {
            ...
            maybeChildren match {
              ...
              case None =>
                maybeCriteria.foreach { criteriaList =>
                  criteriaList.foreach { criteria =>
                    val criteriaId = criteria("criteriaId")
                    logger.info(s"criteria_id: $criteriaId")
          ...
          logger.info("** This observation not attached with any rubrics so skipping processing the domain data \n\n")
        }

        logger.info("===> Processing observation question data")
        ...
        val questionsFunction = new ObservationQuestionFunction(postgresUtil, questionTable)
          ...
          responseType match {
            ...
            case "matrix" =>
              questionsMap.get("value") match {
                ...
                case _ => logger.info("No matrix data found.")
              }
            case _ => logger.info(s"Unsupported responseType: $responseType")
          }
        }
        ...
        answersKey match {
          case answersMap: Map[_, _] =>
            answersMap.foreach { case (_, value) =>
              ...
              logger.info(s"qid: ${questionsMap.get("qid")}")
              ...
              result.foreach { case (themeName, criteriaName) =>
                ...
                logger.info(s"Domain Name: $themeName, Criteria Name: $criteriaName")
              }
              ...
              if (payloadOpt.isDefined) {
                ...
                logger.info(s"Skipping question_id=$question_id as payload is missing.")
              }
            }
          case _ =>
      ...
      if (statusOfSubmission == "completed") {
        ...
        dashboardConfig
          .filter { case (key, _, _) => config.observationReportsEnabled.contains(key) }
              ...
              if (entityType == "admin") {
                ...
                result.foreach { id =>
                  ...
                  if (entityType == "solution") {
                    ...
                    logger.info(s"Inserted [$entityName : $targetedId] with reportType=observation, isRubric=$isRubric, parent_name=${event.entityType}, linked_to=$programId.")
                  } else {
      ...
      def checkExistenceOfFilterData(filterList: List[Map[String, String]], context: ProcessFunction[ObservationEvent, ObservationEvent]#Context, solutionId: String): Unit = {
        ...
        filterList.foreach { filter =>
          ...
          if (tableName.nonEmpty && columnValuePairs.nonEmpty) {
            ...
            if (!dataExists && rowCount > 0) {
              ...
              logger.info(s"As dataExits = $dataExists, rowCount = $rowCount hence pushing the message to kafka.")
              pushObservationDashboardEvents(eventData, context)
            } else {
              logger.info(s"As dataExits = $dataExists, rowCount = $rowCount hence stopping function.")
              return
            }
          } else {
            logger.info(s"table doesn't exits or filter json is empty, hence stopping function.  ")
          }
        }
        logger.info(s">>>>>>>>>>>>>> Completed checking existence of filter data...")
      }

      def pushObservationDashboardEvents(dashboardData: util.HashMap[String, String], context: ProcessFunction[ObservationEvent, ObservationEvent]#Context): util.HashMap[String, AnyRef] = {
        ...
        logger.info(s"----> Pushed new Kafka message to ${config.observationOutputTopic} topic")
