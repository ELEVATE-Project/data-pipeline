...
class UnifiedStreamConfig(override val config: Config) extends BaseJobConfig(config, "StreamProcessorJob") {
  ...
  val projectOutputTopic: String = config.getString("kafka.output.project.topic")
  ...
  val isProjectStreamEnabled: Boolean = if (config.hasPath("kafka.input.project.enabled")) config.getBoolean("kafka.input.project.enabled") else false

  // === Survey ===
  val surveyInputTopic: String = config.getString("kafka.input.survey.topic")
  val surveyOutputTopic: String = config.getString("kafka.output.survey.topic")
  ...
  val isSurveyStreamEnabled: Boolean = if (config.hasPath("kafka.input.survey.enabled")) config.getBoolean("kafka.input.survey.enabled") else false
  ...
  val observationOutputTopic: String = config.getString("kafka.output.observation.topic")
  ...
  val isObservationStreamEnabled: Boolean = if (config.hasPath("kafka.input.observation.enabled")) config.getBoolean("kafka.input.observation.enabled") else false

  // === User ===
  val userInputTopic: String = config.getString("kafka.input.user.topic")
  val userOutputTopic: String = config.getString("kafka.output.user.topic")
  ...
  val userSinkParallelism: Int = config.getInt("task.user.sink.parallelism")
  val usersCleanupHit = "user-cleanup-hit"
  val userMetrics: String = config.getString("postgres.tables.userMetrics")
  val isUserStreamEnabled: Boolean = if (config.hasPath("kafka.input.user.enabled")) config.getBoolean("kafka.input.user.enabled") else false
  ...
  val mentoringSinkParallelism: Int = config.getInt("task.mentoring.sink.parallelism")
  val mentoringCleanupHit = "mentoring-cleanup-hit"
  val isMentoringStreamEnabled: Boolean = if (config.hasPath("kafka.input.mentoring.enabled")) config.getBoolean("kafka.input.mentoring.enabled") else false
