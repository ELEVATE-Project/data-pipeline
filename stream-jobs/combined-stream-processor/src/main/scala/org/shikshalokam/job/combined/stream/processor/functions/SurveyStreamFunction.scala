...
class SurveyStreamFunction(config: UnifiedStreamConfig)(implicit val mapTypeInfo: TypeInformation[SurveyEvent], @transient var postgresUtil: PostgresUtil = null)
    ...
    if (SurveyEvent.status.toLowerCase() == "started" || SurveyEvent.status.toLowerCase() == "inprogress" || SurveyEvent.status.toLowerCase() == "submitted" || SurveyEvent.status.toLowerCase() == "completed") {
      ...
      def insertQuestion(payload: Option[Map[String, Any]], question_id: String, solution_id: String, solution_name: String,
        ...
        if (!payload.exists(_.contains("labels"))) {
          logger.info(s"Skipping question $question_id as 'labels' key is missing.")
        }
      ...
      if (SurveyEvent.status == "completed") {
        ...
        dashboardConfig
          .filter { case (key, _, _) => config.surveyReportsEnabled.contains(key) }
        ...
        case _ =>
          if (entityType == "admin") {
            ...
            result.foreach { id =>
              ...
              logger.info(s"Inserted [$entityName : $targetedId] details.")
  ...
  private def checkExistenceOfFilterData(filterList: List[Map[String, String]], context: ProcessFunction[SurveyEvent, SurveyEvent]#Context, solutionId: String): Unit = {
  ...
  private def pushSurveyDashboardEvents(dashboardData: util.HashMap[String, String], context: ProcessFunction[SurveyEvent, SurveyEvent]#Context): util.HashMap[String, AnyRef] = {
    ...
    context.output(config.surveyOutputTag, event)
    logger.info(s"----> Pushed new Kafka message to ${config.surveyOutputTopic} topic")
