...
import org.shikshalokam.job.mentoring.stream.processor.task.{MentoringStreamConfig, MentoringStreamTask}
...
class MentoringStreamFunctionTestSpec extends BaseTestSpec {
  ...
  val jobConfig: MentoringStreamConfig = new MentoringStreamConfig(config)
  ...
  "Mentoring Stream Job " should "execute successfully " in {
    initialize()
    new MentoringStreamTask(jobConfig, mockKafkaUtil).process()
  }
