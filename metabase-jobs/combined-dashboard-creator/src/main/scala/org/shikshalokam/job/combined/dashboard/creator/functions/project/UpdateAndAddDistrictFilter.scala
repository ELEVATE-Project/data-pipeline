...
object UpdateAndAddDistrictFilter {
  private val logger = LoggerFactory.getLogger(UpdateAndAddDistrictFilter.getClass)
  ...
  def updateAndAddFilter(metabaseUtil: MetabaseUtil, queryResult: JsonNode, stateId: String, districtId: String, collectionId: Int, databaseId: Int, projectTable: String, solutionTable: String): Int = {
    ...
    logger.info(s"** Started to update and create filter questions for district dashboard")
    val objectMapper = new ObjectMapper()
    ...
    logger.info(s"** Completed to update and create filter questions for district dashboard")
    questionId
  }
}