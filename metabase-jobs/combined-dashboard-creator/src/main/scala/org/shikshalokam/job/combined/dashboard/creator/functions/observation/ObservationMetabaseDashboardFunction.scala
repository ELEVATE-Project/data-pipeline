...
class ObservationMetabaseDashboardFunction(config: CombinedDashboardCreatorConfig)(implicit val mapTypeInfo: TypeInformation[ObservationEvent], @transient var postgresUtil: PostgresUtil = null, @transient var metabasePostgresUtil: PostgresUtil = null, @transient var metabaseUtil: MetabaseUtil = null)
  ...
  override def processElement(event: ObservationEvent, context: ProcessFunction[ObservationEvent, ObservationEvent]#Context, metrics: Metrics): Unit = {
    ...
    try {
      logger.info(s"***************** Start of Processing the Metabase Dashboard Event with Id = ${event._id}*****************")
      ...
      val programCollectionName = s"$programName"
        ...
        logger.info(s"domainTable: $observationDomainTable")
        logger.info(s"questionTable: $observationQuestionTable")
        logger.info(s"isRubric: ${event.isRubric}")
        logger.info(s"targetedSolutionId: $targetedSolutionId")
        logger.info(s"targetedProgramId: $targetedProgramId")
        logger.info(s"admin: $admin")
        logger.info(s"programName: $programName")
        logger.info(s"solutionsName: $solutionName")

        logger.info(s">>>>>>>>>>> Started Processing Metabase Observation Dashboards >>>>>>>>>>>>\n")
        ...
        if (solutionName.nonEmpty && targetedProgramId.nonEmpty && programName.nonEmpty) {
          event.reportType match {
            case "Observation" =>
              logger.info(s"===========> Started Processing Metabase Admin Dashboard For Observation \n")
          ...
          logger.info(s"=====> Program name or Solution name is null, skipping the processing")
        }
        ...
        if (filterSync.nonEmpty) {
          ...
          logger.info(s"Finished syncing the $filterTable table ")
        }
        ...
        def createAdminCollection: Int = {
          ...
          if (adminCollectionId != -1) {
            ...
            logger.info(s"Admin Collection returned -1")
            -1
          }
        }
        ...
        def getTheColumnId(databaseId: Int, tableName: String, columnName: String, metabaseUtil: MetabaseUtil, metabasePostgresUtil: PostgresUtil, metabaseApiKey: String, metaTableQuery: String): Int = {
          try {
            ...
            storedColumnIds.get((tableId, columnName)) match {
              ...
              case None =>
                ...
                if (columnId != -1) {
                  ...
                  logger.warn(s"[WARN] $errorMessage")
                  -1
                }
            }
          } catch {
            case e: Exception =>
              ...
              val updateTableQuery = metaTableQuery.replace("'errorMessage'", s"'$escapedError'")
              postgresUtil.insertData(updateTableQuery)
              logger.error(s"[ERROR] Failed to get column ID: ${e.getMessage}")
              -1
          }
        }
        ...
        def extractParameterDicts(parametersQuery: String, entityType: String, databaseId: Int, metabaseUtil: MetabaseUtil, tableName: String, postgresUtil: PostgresUtil, createDashboardQuery: String, metabaseApiKey: String): (Map[String, Int], ListMap[String, String], String, Boolean) = {
          ...
          val (mapOfParamsAfterRemovingBelowEntityParams, _, entityColumnName, isEntityTypeMatched) =
            matchedNodeOpt match {
              ...
              case None =>
                logger.info(s"No entityType match, keeping all params")
        ...
        def validateCollection(collectionName: String, reportFor: String, reportId: Option[String] = None): (Boolean, Int) = {
          ...
          try {
            ...
            case e: Exception =>
              logger.error(s"[ERROR] API or JSON failure: ${e.getMessage}")
              (false, 0)
          }
        }
        ...
        logger.info(s"***************** End of Processing the Metabase Observation Dashboard *****************\n")
      } else {
        logger.info(s"targetedSolutionId is empty or null, skipping dashboard creation.")
      }
    } catch {
      case e: Exception =>
