...
object ComparePage {
  private val logger = LoggerFactory.getLogger(ComparePage.getClass)
  ...
  def ProcessAndUpdateJsonFiles(reportConfigQuery: String, collectionId: Int, databaseId: Int, dashboardId: Int, statenameId: Int, districtnameId: Int, projects: String, solutions: String, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil): ListBuffer[Int] = {
    logger.info(s"=====> Started processing admin level json update function for Region Comparison dashboard")
    ...
    def updateDimension(node: ObjectNode, newId: Int): Unit = {
      if (node.has("dimension") && node.get("dimension").isArray) {
        ...
        if (dimensionNode.size() >= 2) {
          ...
          logger.info(s"Warning: 'dimension' array does not have enough elements to update.")
        }
      } else {
        logger.info(s"Warning: 'dimension' node is missing or not an array.")
      }
    }
    ...
    logger.info(s"=====> Completed processing admin level json update function for Region Comparison dashboard")
    questionCardId
  }
  ...
  def UpdateParameterFunction(metabaseUtil: MetabaseUtil, parametersQuery: String, dashboardId: Int, postgresUtil: PostgresUtil): Unit = {
    logger.info(s"=====> Started processing Region Comparison dashboard parameter")
    val objectMapper = new ObjectMapper()
    ...
    logger.info(s"=====> Completed processing Region Comparison dashboard parameter")
  }
