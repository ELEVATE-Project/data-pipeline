...
object ProcessOrgConstructor {
  private val logger = LoggerFactory.getLogger(ProcessOrgConstructor.getClass)
  def ProcessAndUpdateJsonFiles(reportConfigQuery: String, collectionId: Int, databaseId: Int, dashboardId: Int, statenameId: Int, districtnameId: Int, blocknameId: Int, clusternameId: Int, schoolnameId: Int,
                                tenantUserTable: String, tenantSessionTable: String, tenantSessionAttendanceTable: String, tenantConnectionsTable: String, tenantOrgMentorRatingTable: String, tenantOrgRolesTable: String, orgId: Int,
                                immutableSlugNameToFilterMap: Map[String, Int] = Map.empty, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil): ListBuffer[Int] = {
    logger.info(s"---------------started processing ProcessAndUpdateJsonFiles function----------------")
    val questionCardId = ListBuffer[Int]()
    val objectMapper = new ObjectMapper()

    def processJsonFiles(reportConfigQuery: String, collectionId: Int, databaseId: Int, dashboardId: Int, statenameId: Int, districtnameId: Int, blocknameId: Int, clusternameId: Int, schoolnameId: Int): Unit = {
          ...
          row.get("config") match {
            case Some(queryValue: PGobject) =>
              ...
              if (configJson != null) {
                ...
                val updatedQuestionCard = updateQuestionCardJsonValues(configJson, collectionId, statenameId, districtnameId, blocknameId, clusternameId, schoolnameId, databaseId)
    ...
    def updateQuestionCardJsonValues(configJson: JsonNode, collectionId: Int, statenameId: Int, districtnameId: Int, blocknameId: Int, clusternameId: Int, schoolNameId: Int, databaseId: Int): JsonNode = {
      try {
    ...
    processJsonFiles(reportConfigQuery, collectionId, databaseId, dashboardId, statenameId, districtnameId, blocknameId, clusternameId, schoolnameId)
