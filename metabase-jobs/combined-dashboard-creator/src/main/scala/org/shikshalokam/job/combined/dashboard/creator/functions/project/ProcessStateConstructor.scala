...
object ProcessStateConstructor {
  private val logger = LoggerFactory.getLogger(ProcessStateConstructor.getClass)
  ...
  def ProcessAndUpdateJsonFiles(reportConfigQuery: String, collectionId: Int, databaseId: Int, dashboardId: Int, tabId: Int, stateNameId: Int, districtNameId: Int, programNameId: Int, blockNameId: Int, clusterNameId: Int, orgNameId: Int, projects: String, solutions: String, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil, targetedStateId: String): ListBuffer[Int] = {
    logger.info(s"=====> Started processing state level json update function for Micro Improvements dashboard")
            ...
            case Some(queryValue: PGobject) =>
              ...
              if (rootNode != null) {
                ...
                cardIdOpt match {
                  ...
                  case None =>
                    logger.error(s"Error: Unable to extract card ID for $chartName. Skipping...")
                }
              } else {
                logger.warn("Warning: File could not be parsed as JSON. Skipping...")
              }

            case Some(_) =>
              logger.warn("Unexpected type for 'query' key value.")

            case None =>
              logger.warn("Key 'query' not found in the result row.")
          }
        } else {
          row.get("config") match {
            case Some(queryValue: PGobject) =>
    ...
    def updateJsonWithCardId(json: JsonNode, cardId: Int, dashboardId: Int, tabId: Int): Option[JsonNode] = {
      Try {
    ...
    logger.info(s"=====> Completed processing state level json update function for Micro Improvements dashboard")
    questionCardId
  }
}
