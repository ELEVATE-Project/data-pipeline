...
class UserMetabaseDashboardFunction(config: CombinedDashboardCreatorConfig)(implicit val mapTypeInfo: TypeInformation[UserEvent], @transient var postgresUtil: PostgresUtil = null, @transient var metabasePostgresUtil: PostgresUtil = null, @transient var metabaseUtil: MetabaseUtil = null)
  ...
  override def processElement(event: UserEvent, context: ProcessFunction[UserEvent, UserEvent]#Context, metrics: Metrics): Unit = {
    ...
    try {
      logger.info(s"***************** Start of Processing the User Metabase Dashboard Event with = ${event.tenantCode} *****************")
      ...
      if (reportAdminPresent && reportAdminCollectionId != 0) {
        logger.info(s"=====> 'User Activity' collection present with id: $reportAdminCollectionId, Skipping this step.")
      } else {
      ...
      if (tenantCode.nonEmpty) {
        ...
        if (tenantCollectionPresent && tenantCollectionId != 0) {
          logger.info(s"=====> '$tenantAdminCollectionName' collection present with id: $tenantCollectionId, Skipping this step.")
        } else {
      ...
      def createUserMetricsCollectionAndDashboardForAdmin(): Unit = {
        ...
        if (collectionId != -1) {
          ...
          logger.info(s"=====> Dashboard '$dashboardName' created and metadata updated successfully.")
        }
      }
      ...
      logger.info(s"***************** End of Processing the User Metabase Dashboard Event with = ${event.tenantCode} *****************")
    } catch {
      case e: Exception =>
