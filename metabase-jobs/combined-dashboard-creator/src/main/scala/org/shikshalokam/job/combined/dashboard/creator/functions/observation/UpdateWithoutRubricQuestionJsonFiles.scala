...
object UpdateWithoutRubricQuestionJsonFiles {
  private val logger = LoggerFactory.getLogger(UpdateWithoutRubricQuestionJsonFiles.getClass)
              ...
              if (cleanedJson != null) {
                ...
                updatedQuestionIdInDashCard.foreach { node =>
                  ...
                  if (!dashCardsNode.isMissingNode && !dashCardsNode.isNull) {
                    ...
                    logger.info("No 'dashCards' key found in the JSON.")
                  }
                }
                logger.info(s"Added question card with ID: $cardId and chart name: $chartName to dashboard with ID: $dashboardId")
              }
            case None =>
              logger.info("Key 'config' not found in the result row.")
          }
        }
        else {
          row.get("config") match {
      ...
      def processHeading(questionText: String): Unit = {
        results("heading").foreach { headingRow =>
          headingRow.get("config") match {
            ...
            case None => logger.info("Key 'config' not found in the heading result row.")
          }
        }
      }
      ...
      def processQuestionType(questionType: String, questionId: String): Unit = {
        ...
        results(resultKey).foreach { row =>
          row.get("config") match {
            ...
            case None => logger.info(s"Key 'config' not found in the $resultKey result row.")
          }
        }
      }
    ...
    def cleanDashboardJson(jsonStr: String, newLevelDict: Map[String, String]): String = {
      ...
      if (queryNode != null && queryNode.isTextual) {
        ...
        newLevelDict.keys.foreach { key =>
          ...
          if (before != queryStr) {
            logger.info(s"Removed filter for key: $key")
          } else {
            logger.info(s"No filter found for key: $key")
          }
        }
