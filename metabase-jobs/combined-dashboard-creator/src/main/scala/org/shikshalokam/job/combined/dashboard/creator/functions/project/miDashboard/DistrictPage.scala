package org.shikshalokam.job.combined.dashboard.creator.functions.project.miDashboard
...
object DistrictPage {
  private val logger = LoggerFactory.getLogger(DistrictPage.getClass)
  ...
  def ProcessAndUpdateJsonFiles(reportConfigQuery: String, collectionId: Int, databaseId: Int, dashboardId: Int, projects: String, solutions: String, reportConfig: String, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil, targetedDistrictId: String, targetedDistrictName: String): ListBuffer[Int] = {
    logger.info(s"=====> Started processing district level json update function for District Overview dashboard")
            ...
            case Some(queryValue: PGobject) =>
              ...
              if (rootNode != null) {
                ...
                cardIdOpt match {
                  ...
                  case None =>
                    logger.info(s"Error: Unable to extract card ID for $chartName. Skipping...")
                }
              } else {
                logger.info("Warning: File could not be parsed as JSON. Skipping...")
              }

            case Some(_) =>
              logger.info("Unexpected type for 'query' key value.")

            case None =>
              logger.info("Key 'query' not found in the result row.")
          }
        } else {
          row.get("config") match {
            case Some(queryValue: PGobject) =>
    ...
    logger.info(s"=====> Completed processing district level json update function for District Overview dashboard")
    questionCardId
  }
    ...
    def getTheQuestionId(json: JsonNode): Int = {
      try {
        ...
        Option(responseJson.get("id")).map(_.asInt()).getOrElse {
          logger.info("Error: 'id' field not found in the response.")
          -1
        }
      } catch {
        case ex: Exception =>
          logger.info(s"Error fetching 'id' from response: ${ex.getMessage}")
          -1
      }
    }
  ...
  def updateParameterFunction(metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil, parametersQuery: String, slugNameToStateIdMap: Map[String, Int], dashboardId: Int): Unit = {
    logger.info(s"=====> Started Processing District Overview dashboard parameter")
    val objectMapper = new ObjectMapper()
    ...
    val updatedParameterJson = try {
          ...
          if (param.isInstanceOf[ObjectNode]) {
            ...
            slugNameToStateIdMap.get(slug) match {
              case Some(newCardId) =>
                ...
                Some(updatedParam)
              case None =>
                logger.info(s"No card_id found for slug '$slug', skipping update")
                None
            }
          } else {
            logger.info(s"Skipping param as it is not of type ObjectNode: ${param.toString}")
            None
          }
        } catch {
          case e: Exception =>
            logger.info(s"Error processing param: ${param.toString}")
      ...
      case e: Exception =>
        logger.info(s"Error during JSON processing: ${e.getMessage}")
        e.printStackTrace()
        List.empty
    }
    ...
    logger.info(s"=====> Completed Processing District Overview dashboard parameter")
  }
