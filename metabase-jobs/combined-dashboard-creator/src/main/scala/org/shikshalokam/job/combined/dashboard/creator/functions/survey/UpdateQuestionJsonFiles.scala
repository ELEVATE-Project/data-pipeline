...
object UpdateQuestionJsonFiles {
  private val logger = LoggerFactory.getLogger(UpdateQuestionJsonFiles.getClass)
  def ProcessAndUpdateJsonFiles(collectionId: Int, databaseId: Int, dashboardId: Int, tabId: Int, stateNameId: Int, districtNameId: Int, blockNameId: Int, clusterNameId: Int, schoolNameId: Int, orgNameId: Int, question: String, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil, report_config: String, evidenceBaseUrl: String): ListBuffer[Int] = {
    logger.info(s"---------------started processing ProcessAndUpdateJsonFiles function----------------")
              ...
              if (configJson != null) {
                ...
                updatedQuestionIdInDashCard.foreach { node =>
                  ...
                  if (!dashCardsNode.isMissingNode && !dashCardsNode.isNull) {
                    ...
                    logger.warn("No 'dashCards' key found in the JSON.")
                  }
                }
              }
            case None =>
              logger.warn("Key 'config' not found in the result row.")
          }
        }
        else {
          row.get("config") match {
    ...
    def processJsonFiles(collectionId: Int, databaseId: Int, dashboardId: Int, tabId: Int, stateNameId: Int, districtNameId: Int, blockNameId: Int, clusterNameId: Int, schoolNameId: Int, orgNameId: Int, question: String, report_config: String, evidenceBaseUrl: String): Unit = {
      ...
      val queries = Map(
        ...
        "slider" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'slider-chart';",
        "radio" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'radio-chart';",
        "multiselect" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'multiselect-chart';",
        "numbers" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'number-chart';",
        "text" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'text-chart';",
        "heading" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'heading';",
        "date" -> s"SELECT * FROM $report_config WHERE dashboard_name = 'Survey' AND question_type = 'date-chart';"
      )
      ...
      def processHeading(questionText: String): Unit = {
        results("heading").foreach { headingRow =>
          headingRow.get("config") match {
            ...
            case None => logger.warn("Key 'config' not found in the heading result row.")
          }
        }
      }
          ...
          row.get("config") match {
            case Some(queryValue: PGobject) =>
              ...
              Option(configJson).foreach { json =>
                ...
                val requestBody = finalQuestionCard.asInstanceOf[ObjectNode]
                val cardId = mapper.readTree(metabaseUtil.createQuestionCard(requestBody.toString)).path("id").asInt()
                logger.info(s">>>>>>>>> Successfully created question card with card_id: $cardId")
                questionCardId.append(cardId)
            ...
            case None => logger.warn(s"Key 'config' not found in the $resultKey result row.")
          }
        }
      }
      ...
      results("nonMatrix").foreach { row =>
        ...
        logger.info(s"Formatted Question Text: $formattedText")
    ...
    def updateDimension(node: JsonNode, newId: Int): Unit = {
      if (node.has("dimension") && node.get("dimension").isArray) {
        ...
        if (dimensionNode.size() >= 2) {
          ...
          logger.warn(s"Warning: 'dimension' array does not have enough elements to update.")
        }
      } else {
        logger.warn(s"Warning: 'dimension' node is missing or not an array.")
      }
    }
    ...
    processJsonFiles(collectionId, databaseId, dashboardId, tabId, stateNameId, districtNameId, blockNameId, clusterNameId, schoolNameId, orgNameId, question, report_config, evidenceBaseUrl)
    logger.info(s"---------------processed ProcessAndUpdateJsonFiles function----------------")
    questionCardId
  }
}
