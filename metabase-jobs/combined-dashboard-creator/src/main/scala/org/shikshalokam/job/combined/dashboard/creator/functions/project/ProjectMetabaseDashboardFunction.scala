...
import org.shikshalokam.job.combined.dashboard.creator.functions.project.miDashboard.{ComparePage, DistrictPage, HomePage, StatePage}
...
class ProjectMetabaseDashboardFunction(config: CombinedDashboardCreatorConfig)(implicit val mapTypeInfo: TypeInformation[ProjectEvent], @transient var postgresUtil: PostgresUtil = null, @transient var metabasePostgresUtil: PostgresUtil = null, @transient var metabaseUtil: MetabaseUtil = null)
  ...
  override def processElement(event: ProjectEvent, context: ProcessFunction[ProjectEvent, ProjectEvent]#Context, metrics: Metrics): Unit = {
    ...
    try {
      logger.info(s"***************** Start of Processing the Metabase Project Dashboard Event with Id = ${event._id} *****************")
      ...
      logger.info(s"Targeted State ID: $targetedStateId")
      logger.info(s"Targeted District ID: $targetedDistrictId")
      ...
      logger.info(s"Targeted Solution Name: $solutionName")

      event.reportType match {
        case "Project" =>
          logger.info(s">>>>>>>>>>> Started Processing Metabase Project Dashboards >>>>>>>>>>>>")
          ...
          if (targetedStateId.nonEmpty && stateName.nonEmpty) {
            ...
            if (collectionPresent && collectionId != 0) {
              ...
              if (stateDashboardPresent && stateDashboardId != 0) {
                logger.info(s"=====> $stateName - State overview dashboard already present inside National Overview collection with id: $stateDashboardId, Skipping this step.")
              } else {
                val (stateDashboardName, stateDashboardDescription) = (s"$stateName - State overview", s"This dashboard contains important metrics for $stateName state\n\nDashboard For: Admin\n\nState Id: $targetedStateId")
            ...
            logger.info("Targeted State is not present or is empty")
          }
          ...
          if (targetedDistrictId.nonEmpty && districtName.nonEmpty) {
            ...
            if (districtCollectionPresent && districtCollectionId != 0) {
              ...
              logger.info("=====> Creating $districtName district Collection And Dashboard")
            ...
            if (stateIdForDistrictId.nonEmpty && stateNameForDistrictId.nonEmpty) {
              ...
              if (stateCollectionPresent && stateCollectionId != 0) {
                ...
                if (districtDashboardPresent && districtDashboardId != 0) {
                  logger.info(s"=====> $districtName District [Tenant : $tenantIdForDistrictId] dashboard already present inside state collection with id: $districtDashboardId, Skipping this step.")
                } else {
            ...
            if (collectionPresent && collectionId != 0) {
              ...
              if (districtDashboardPresent && districtDashboardId != 0) {
                logger.info(s"=====> $districtName District [Tenant : $tenantIdForDistrictId] dashboard already present inside National Overview collection, Skipping this step.")
              } else {
                val (districtDashboardName, districtDashboardDescription) = (s"$districtName District [Tenant : $tenantIdForDistrictId]", s"This dashboard contains important metrics for $districtName district in $stateName state\n\nDashboard For: Admin\n\nDistrict Id: $targetedDistrictId")
            ...
            logger.info("Targeted District is not present or is empty")
          }
      ...
      def createStateOverviewDashboard(stateDashboardName: String, stateDashboardDescription: String, parentCollectionId: Int, parentCollectionName: String, stateName: String, databaseId: Int, stateReportConfigQuery: String, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil, processType: String, reportFor: String, pinDashboard: String): Unit = {
        ...
        if (parentCollectionId != -1) {
          ...
          logger.info(s"Parent collection ID is -1 for $stateName state dashboard hence skipping the creation of state dashboard.")
        }
      }
      ...
      def createDistrictOverviewDashboard(districtDashboardName: String, dashboardDescription: String, parentCollectionId: Int, parentCollectionName: String, districtName: String, databaseId: Int, metabaseUtil: MetabaseUtil, postgresUtil: PostgresUtil, processType: String, reportFor: String, pinDashboard: String): Unit = {
        ...
        if (parentCollectionId != -1) {
          ...
          logger.info(s"Parent collection ID is -1 for $parentCollectionName hence skipping dashboard creation.")
        }
      }
      ...
      def createSolutionCollectionAndDashboard(programCollectionId: Int, solutionCollectionName: String, solutionCollectionDescription: String, reportFor: String): Unit = {
        ...
        logger.info(s">>> Created Solution collection with Id: $solutionCollectionId inside the parent collection Id: $programCollectionId")
        ...
        if (projectTabId != -1 && submissionCsvTabId != -1 && taskReportCsvTabId != -1 && statusReportCsvTabId != -1) {
          ...
          logger.info(s"Creating tabs failed please check: $projectTabId, $submissionCsvTabId, $taskReportCsvTabId", statusReportCsvTabId)
        }
      }
      ...
      def getTheColumnId(databaseId: Int, tableName: String, columnName: String, metabaseUtil: MetabaseUtil, metabasePostgresUtil: PostgresUtil, metabaseApiKey: String, metaTableQuery: String): Int = {
        try {
          ...
          storedColumnIds.get((tableId, columnName)) match {
            ...
            case None =>
              ...
              if (columnId != -1) {
                ...
                logger.info(s"[WARN] $errorMessage")
                -1
              }
          }
        } catch {
          case e: Exception =>
            ...
            val updateTableQuery = metaTableQuery.replace("'errorMessage'", s"'$escapedError'")
            postgresUtil.insertData(updateTableQuery)
            logger.info(s"[ERROR] Failed to get column ID: ${e.getMessage}")
            -1
        }
      }
      ...
      def validateCollection(collectionName: String, reportFor: String, reportId: Option[String] = None): (Boolean, Int) = {
        ...
        logger.info(s">>> Checking Metabase API for collection: $collectionName")
        try {
          ...
          logger.info(s">>> API result: $result")
          result
        } catch {
          case e: Exception =>
            logger.info(s"[ERROR] API or JSON failure: ${e.getMessage}")
            (false, 0)
        }
      }
      ...
      def validateDashboard(dashboardName: String, reportFor: String, reportId: Option[String] = None): (Boolean, Int) = {
        val mapper = new ObjectMapper()
        logger.info(s">>> Checking Metabase API for dashboard: $dashboardName")
        try {
      ...
      logger.info(s"***************** End of Processing the Metabase Project Dashboard *****************\n")
    } catch {
      case e: Exception =>
