...
object AddMetadataFilter {
  private val logger = LoggerFactory.getLogger(AddMetadataFilter.getClass)
  val objectMapper = new ObjectMapper()

  def updateAndAddFilter(metabaseUtil: MetabaseUtil, queryResult: JsonNode, collectionId: Int, databaseId: Int, tenantUserMetadataTable: String): Int = {
    logger.info(s"** Started to add filter questions for metadata")
    val objectMapper = new ObjectMapper()

    def replaceTableName(json: JsonNode, tenantUserMetadataTable: String): JsonNode = {
          ...
          case obj: ObjectNode =>
            obj.fieldNames().forEachRemaining { fieldName =>
              ...
              if (childNode.isTextual) {
                ...
                if (updatedText.contains("${tenantUserMetadataTable}")) {
                  updatedText = updatedText.replace("${tenantUserMetadataTable}", tenantUserMetadataTable)
                }
    ...
    def getTheQuestionId(json: JsonNode): Int = {
      try {
        ...
        Option(responseJson.get("id")).map(_.asInt()).getOrElse {
          logger.error("Error: 'id' field not found in the response.")
          -1
        }
      } catch {
        case ex: Exception =>
          logger.error(s"Error fetching 'id' from response: ${ex.getMessage}")
          -1
      }
    }


    val ReplacedStateNameJson = replaceTableName(queryResult, tenantUserMetadataTable)
    ...
    logger.info(s"** Completed to update and create filter questions for dashboard")
    questionId
  }
}