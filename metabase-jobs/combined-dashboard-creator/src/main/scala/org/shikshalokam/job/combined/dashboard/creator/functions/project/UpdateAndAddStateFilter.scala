...
object UpdateAndAddStateFilter {
  private val logger = LoggerFactory.getLogger(UpdateAndAddStateFilter.getClass)
  ...
  def updateAndAddFilter(metabaseUtil: MetabaseUtil, queryResult: JsonNode, stateId: String, collectionId: Int, databaseId: Int, projectTable: String, solutionTable: String): Int = {
    logger.info(s"** Started to update and create filter questions for state dashboard")
    val objectMapper = new ObjectMapper()
    ...
    logger.info(s"** Completed to update and create filter questions for state dashboard")
    questionId
  }
}